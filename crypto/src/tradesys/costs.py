...
Lives at the top level rather than under ``research/`` because both the
research environment and the simulated venue need it, and a venue adapter
importing from ``research`` would make the package graph circular. That is not
a workaround: cost arithmetic is domain knowledge the backtest and the live
path must agree on, which is the whole content of SPEC section 3.2 rule 2.
